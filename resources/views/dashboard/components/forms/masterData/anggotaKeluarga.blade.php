<div class="row g-3">
    ...
    <div class="col-lg-6 col-md-6">
        ...
        @slot('options')
            @foreach ($agama as $row)
                <option value="{{ $row->id }}" {{ isset($anggotaKeluarga) && $row->id == $anggotaKeluarga->agama_id ? 'selected' : '' }}>{{ $row->agama }}</option>                                       
            @endforeach                         
        @endslot
        @endcomponent
    </div>
    <div class="col-lg-6 col-md-6">
        ...
        @slot('options')
            @foreach ($pendidikan as $row)
                <option value="{{ $row->id }}" {{ isset($anggotaKeluarga) && $row->id == $anggotaKeluarga->pendidikan_id ? 'selected' : '' }}>{{ $row->pendidikan }}</option>                                       
            @endforeach         
        @endslot
        @endcomponent
    </div>
    <div class="col-lg-6 col-md-6">
        ...
        @slot('options')
            @foreach ($pekerjaan as $row)
                <option value="{{ $row->id }}" {{ isset($anggotaKeluarga) && $row->id == $anggotaKeluarga->jenis_pekerjaan_id ? 'selected' : '' }}>{{ $row->pekerjaan }}</option>                                       
            @endforeach   
        @endslot
        @endcomponent
    </div>
    <div class="col-lg-6 col-md-6">
        ...
        @slot('options')
            @foreach ($golonganDarah as $row)
                <option value="{{ $row->id }}" {{ isset($anggotaKeluarga) && $row->id == $anggotaKeluarga->golongan_darah_id ? 'selected' : '' }}>{{ $row->golongan_darah }}</option>                                       
            @endforeach   
        @endslot
        @endcomponent
    </div>
    <div class="col-lg-6 col-md-6">
        ...
        @slot('options')
            @foreach ($statusPerkawinan as $row)
                <option value="{{ $row->id }}" {{ isset($anggotaKeluarga) && $row->id == $anggotaKeluarga->status_perkawinan_id ? 'selected' : '' }}>{{ $row->status_perkawinan }}</option>                                       
            @endforeach   
        @endslot
        @endcomponent
    </div>
    ...
    <div class="col-lg-6 col-md-6">
        ...
        @slot('options')
            @foreach ($statusHubungan as $row)
                <option value="{{ $row->id }}" {{ isset($anggotaKeluarga) && $row->id == $anggotaKeluarga->status_hubungan_dalam_keluarga_id ? 'selected' : '' }}>{{ $row->status_hubungan }}</option>                                       
            @endforeach   
        @endslot
        @endcomponent
    </div>
    ...
    <div class="col-lg-6 col-md-6">
        ...
        <div class="col-md-9 col-sm-8">
            <div class="image-input avatar xxl rounded-4" style="background-image: url({{ isset($anggotaKeluarga) && $anggotaKeluarga->foto_profil != NULL ? asset('upload/foto_profil/keluarga/'.$anggotaKeluarga->foto_profil) : asset('assets/dashboard/images/avatar.png') }});">
                <div class="avatar-wrapper rounded-4" style="background-image: url({{ isset($anggotaKeluarga) && $anggotaKeluarga->foto_profil != NULL ? asset('upload/foto_profil/keluarga/'.$anggotaKeluarga->foto_profil) : asset('assets/dashboard/images/avatar.png') }});"></div>
                <div class="file-input" style="background: var(--card-color); text-align: center; height: 24px; width: 24px; line-height: 24px; border-radius: 24px; background-position: center !important;">
                    <input type="file" class="form-control pb-3" name="foto_profil" id="foto-profil" style="z-index: 999999" value="" accept="image/*">
...
<div class="card fieldset mt-5 p-4">
    ...
    <div class="row g-3">
            ...
            @component('dashboard.components.formElements.select', [
                ...
                @slot('options')
                    @foreach ($provinsi as $prov)
                        <option value="{{ $prov->id }}" {{ isset($anggotaKeluarga->wilayahDomisili) && $prov->id == $anggotaKeluarga->wilayahDomisili->provinsi_id ? 'selected' : '' }}>{{ $prov->nama }}</option>                                       
                    @endforeach 
                @endslot
            @endcomponent
        </div>
                ...
                @slot('options')
                    @if ($kabupatenKotaDomisili)
                        @foreach ($kabupatenKotaDomisili as $kab)
                            <option value="{{ $kab->id }}" {{ isset($anggotaKeluarga->wilayahDomisili) && $kab->id == $anggotaKeluarga->wilayahDomisili->kabupaten_kota_id ? 'selected' : '' }}>{{ $kab->nama }}</option>                                       
                        @endforeach
                    @endif
                @endslot
            @endcomponent
                ...
                @slot('options')
                    @if ($kecamatanDomisili)
                        @foreach ($kecamatanDomisili as $kec)
                            <option value="{{ $kec->id }}" {{ isset($anggotaKeluarga->wilayahDomisili) && $kec->id == $anggotaKeluarga->wilayahDomisili->kecamatan_id ? 'selected' : '' }}>{{ $kec->nama }}</option>                                       
                        @endforeach
                    @endif
                @endslot
            @endcomponent
                ...
                @slot('options')
                    @if ($desaKelurahanDomisili)
                        @foreach ($desaKelurahanDomisili as $des)
                            <option value="{{ $des->id }}" {{ isset($anggotaKeluarga->wilayahDomisili) && $des->id == $anggotaKeluarga->wilayahDomisili->desa_kelurahan_id ? 'selected' : '' }}>{{ $des->nama }}</option>                                       
                        @endforeach
                    @endif
                @endslot
            @endcomponent
        ...
        <div class="col-12" id="col-alamat-domisili">
            ...
            <input class="form-control" type="file" id="file-domisili" name="file_domisili" accept="image/*, application/pdf">
            @if (isset($anggotaKeluarga))
                <small class="text-muted mt-1" style="font-style: italic">Boleh dikosongkan apabila sebelumnya telah mengupload Surat Keterangan Domisili dan tidak ingin mengubahnya</small>
            @endif
...
@push('script')
    <script>
        ...
        $('#check-domisili').click(function() {
            if($(this).is(':checked')) {
                if(alamatKK == '' || provinsiKK == '' || kabupatenKotaKK == '' || kecamatanKK == '' || desaKelurahanKK == '') {
                    Swal.fire({
                        icon: 'error',
        ...
        $(document).on('change','#provinsi-domisili', function(){
            ...
            $.get("{{ route('listKabupatenKota') }}", {idProvinsi: $("#provinsi-domisili").val()}, function(result) {
                $.each(result, function(key, val) {
                    if(val.id == kabupatenKotaKK) {
                        if($('#check-domisili').is(':checked')) {
                            $('#kabupaten-kota-domisili').append('<option value="'+val.id+'" selected>'+val.nama+'</option>')
                            $('#kabupaten-kota-domisili').trigger('change')
                        } else{
                            $('#kabupaten-kota-domisili').append(`<option value="${val.id}">${val.nama}</option>`);
                        }
                    } else {
                        $('#kabupaten-kota-domisili').append(`<option value="${val.id}">${val.nama}</option>`);
        ...
        $(document).on('change','#kabupaten-kota-domisili', function(){
            ...
            $.get("{{ route('listKecamatan') }}", {idKabupatenKota: $("#kabupaten-kota-domisili").val()}, function(result) {
                $.each(result, function(key, val) {
                    if(val.id == kecamatanKK) {
                        if($('#check-domisili').is(':checked')) {
                            $('#kecamatan-domisili').append(`<option value="${val.id}" selected>${val.nama}</option>`);
                            $('#kecamatan-domisili').trigger('change')
                        } else{
                            $('#kecamatan-domisili').append(`<option value="${val.id}">${val.nama}</option>`);
                        }
                    } else {
                        $('#kecamatan-domisili').append(`<option value="${val.id}">${val.nama}</option>`);
                    }
        ...
        $(document).on('change','#kecamatan-domisili', function(){
            ...
            $.get("{{ route('listDesaKelurahan') }}", {idKecamatan: $("#kecamatan-domisili").val()}, function(result) {
                $.each(result, function(key, val) {
                    if(val.id == desaKelurahanKK) {
                        if($('#check-domisili').is(':checked')) {
                            $('#desa-kelurahan-domisili').append(`<option value="${val.id}" selected>${val.nama}</option>`);
                            $('#desa-kelurahan-domisili').trigger('change')
                        } else{
                            $('#desa-kelurahan-domisili').append(`<option value="${val.id}">${val.nama}</option>`);
                        }
