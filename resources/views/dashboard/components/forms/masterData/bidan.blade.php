...
<form id="{{ $form_id }}" action="#" method="POST" enctype="multipart/form-data" autocomplete="off">
    ...
    <div class="row g-4">
            ...
            @component('dashboard.components.formElements.select',
                ...
                @slot('options')
                    @foreach ($agama as $row)
                        <option value="{{ $row->id }}"
                            {{ isset($bidan) && $row->id == $bidan->agama_id ? 'selected' : '' }}>{{ $row->agama }}</option>
                    @endforeach
                @endslot
            @endcomponent
        </div>
        ...
        <div class="col-lg-4 col-md-6" id="col-nomor-hp">
            @component('dashboard.components.formElements.input',
                [
                    'label' => 'Nomor HP',
                    'type' => 'text',
                ...
                @slot('options')
                    @foreach ($provinsi as $prov)
                        <option value="{{ $prov->id }}"
                            {{ isset($bidan) && $prov->id == $bidan->provinsi_id ? 'selected' : '' }}>{{ $prov->nama }}
                        </option>
                    @endforeach
                @endslot
            @endcomponent
                ...
                @slot('options')
                    @if ($method == 'PUT')
                        @foreach ($kabupatenKota as $kab)
                            <option value="{{ $kab->id }}"
                                {{ isset($bidan) && $kab->id == $bidan->kabupaten_kota_id ? 'selected' : '' }}>
                                {{ $kab->nama }}</option>
                        @endforeach
                    @endif
                @endslot
                ...
                @slot('options')
                    @if ($method == 'PUT')
                        @foreach ($kecamatan as $kec)
                            <option value="{{ $kec->id }}"
                                {{ isset($bidan) && $kec->id == $bidan->kecamatan_id ? 'selected' : '' }}>
                                {{ $kec->nama }}</option>
                        @endforeach
                    @endif
                @endslot
                ...
                @slot('options')
                    @if ($method == 'PUT')
                        @foreach ($desaKelurahan as $des)
                            <option value="{{ $des->id }}"
                                {{ isset($bidan) && $des->id == $bidan->desa_kelurahan_id ? 'selected' : '' }}>
                                {{ $des->nama }}</option>
                        @endforeach
                    @endif
                @endslot
        ...
        <div class="col-lg-4 col-md-6">
            ...
            <div class="col-md-9 col-sm-8">
                <div class="image-input avatar xxl rounded-4"
                    style="background-image: url({{ isset($bidan) && $bidan->foto_profil != null ? Storage::url('upload/foto_profil/bidan/' . $bidan->foto_profil) : asset('assets/dashboard/images/avatar.png') }});">
