<div class="row g-3 py-1">
        ...
        @component('dashboard.components.formElements.select', [
            ...
            @slot('options')
                @foreach ($provinsi as $prov)
                    <option value="{{ $prov->id }}" {{ isset($kartuKeluarga) && $prov->id == $kartuKeluarga->provinsi_id ? 'selected' : '' }}>{{ $prov->nama }}</option>                                       
                @endforeach 
            @endslot
        @endcomponent
    </div>
            ...
            @slot('options')
                @if ($kartuKeluarga)
                    @foreach ($kabupatenKota as $kab)
                        <option value="{{ $kab->id }}" {{ isset($kartuKeluarga) && $kab->id == $kartuKeluarga->kabupaten_kota_id ? 'selected' : '' }}>{{ $kab->nama }}</option>                                       
                    @endforeach
                @endif
            @endslot
        @endcomponent
            ...
            @slot('options')
                @if ($kartuKeluarga)
                    @foreach ($kecamatan as $kec)
                        <option value="{{ $kec->id }}" {{ isset($kartuKeluarga) && $kec->id == $kartuKeluarga->kecamatan_id ? 'selected' : '' }}>{{ $kec->nama }}</option>                                       
                    @endforeach
                @endif
            @endslot
        @endcomponent
            ...
            @slot('options')
                @if ($kartuKeluarga)
                    @foreach ($desaKelurahan as $des)
                        <option value="{{ $des->id }}" {{ isset($kartuKeluarga) && $des->id == $kartuKeluarga->desa_kelurahan_id ? 'selected' : '' }}>{{ $des->nama }}</option>                                       
                    @endforeach
                @endif
            @endslot
        @endcomponent
    ...
    <div class="col-12">
        ...
        <input class="form-control" name="file_kartu_keluarga" type="file" id="file-kartu-keluarga" accept="image/*, application/pdf">
        @if ($kartuKeluarga)
            <small class="text-muted mt-1" style="font-style: italic">Boleh dikosongkan apabila sebelumnya telah mengupload Kartu Keluarga dan tidak ingin mengubahnya</small>
        @endif
