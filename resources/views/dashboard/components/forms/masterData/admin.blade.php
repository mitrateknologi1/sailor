            ...
            @component('dashboard.components.formElements.select',
                ...
                @slot('options')
                    @foreach ($agama as $row)
                        <option value="{{ $row->id }}"
                            {{ isset($admin) && $row->id == $admin->agama_id ? 'selected' : '' }}>{{ $row->agama }}</option>
                    @endforeach
                @endslot
            @endcomponent
        </div>
                ...
                @slot('options')
                    @if ($method == 'PUT')
                        @foreach ($kabupatenKota as $kab)
                            <option value="{{ $kab->id }}"
                                {{ isset($admin) && $kab->id == $admin->kabupaten_kota_id ? 'selected' : '' }}>
                                {{ $kab->nama }}</option>
                        @endforeach
                    @endif
                @endslot
                ...
                @slot('options')
                    @if ($method == 'PUT')
                        @foreach ($kecamatan as $kec)
                            <option value="{{ $kec->id }}"
                                {{ isset($admin) && $kec->id == $admin->kecamatan_id ? 'selected' : '' }}>
                                {{ $kec->nama }}</option>
                        @endforeach
                    @endif
                @endslot
                ...
                @slot('options')
                    @if ($method == 'PUT')
                        @foreach ($desaKelurahan as $des)
                            <option value="{{ $des->id }}"
                                {{ isset($admin) && $des->id == $admin->desa_kelurahan_id ? 'selected' : '' }}>
                                {{ $des->nama }}</option>
                        @endforeach
                    @endif
                @endslot
        ...
        <div class="col-lg-4 col-md-6">
            ...
            <div class="col-md-9 col-sm-8">
                <div class="image-input avatar xxl rounded-4"
                    style="background-image: url({{ isset($admin) && $admin->foto_profil != null ? Storage::url('upload/foto_profil/admin/' . $admin->foto_profil) : asset('assets/dashboard/images/avatar.png') }});">
