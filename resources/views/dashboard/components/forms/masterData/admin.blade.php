    ...
    <div class="row g-4">
            ...
            <div class="d-flex flex-row">
                <div class="p-2">
                    @component('dashboard.components.formElements.radio', [
                        ...
                        'value' => 'LAKI-LAKI',
                        'label' => 'LAKI-LAKI',
                        'checked' => isset($admin) && $admin->jenis_kelamin == 'LAKI-LAKI' ? 'checked' : '',
                        ])
                    @endcomponent
                </div>
                <div class="p-2">
                    @component('dashboard.components.formElements.radio', [
                        ...
                        'value' => 'PEREMPUAN',
                        'label' => 'PEREMPUAN',
                        'checked' => isset($admin) && $admin->jenis_kelamin == 'PEREMPUAN' ? 'checked' : '',
                        ])
                    @endcomponent
                </div>
            </div>
        ...
        <div class="col-lg-4 col-md-6">
            @component('dashboard.components.formElements.input', [
                ...
                'value' => $admin->tanggal_lahir ?? null,
                'wajib' => '<sup class="text-danger">*</sup>',
                ])
            @endcomponent
        </div>
        <div class="col-lg-4 col-md-6">
            ...
            @slot('options')
                @foreach ($agama as $row)
                    <option value="{{ $row->id }}" {{ isset($admin) && $row->id == $admin->agama_id ? 'selected' : '' }}>{{ $row->agama }}</option>                                       
                @endforeach  
            @endslot
            @endcomponent
        </div>
            ...
            @component('dashboard.components.formElements.select', [
                ...
                @slot('options')
                    @foreach ($provinsi as $prov)
                        <option value="{{ $prov->id }}" {{ isset($admin) && $prov->id == $admin->provinsi_id ? 'selected' : '' }}>{{ $prov->nama }}</option>                                       
                    @endforeach
                @endslot
            @endcomponent
        </div>
                ...
                @slot('options')
                    @if ($method == 'PUT')
                        @foreach ($kabupatenKota as $kab)
                            <option value="{{ $kab->id }}" {{ isset($admin) && $kab->id == $admin->kabupaten_kota_id ? 'selected' : '' }}>{{ $kab->nama }}</option>                                       
                        @endforeach
                    @endif
                @endslot
            @endcomponent
                ...
                @slot('options')
                    @if ($method == 'PUT')
                        @foreach ($kecamatan as $kec)
                            <option value="{{ $kec->id }}" {{ isset($admin) && $kec->id == $admin->kecamatan_id ? 'selected' : '' }}>{{ $kec->nama }}</option>                                       
                        @endforeach
                    @endif
                @endslot
            @endcomponent
                ...
                @slot('options')
                    @if ($method == 'PUT')
                        @foreach ($desaKelurahan as $des)
                            <option value="{{ $des->id }}" {{ isset($admin) && $des->id == $admin->desa_kelurahan_id ? 'selected' : '' }}>{{ $des->nama }}</option>                                       
                        @endforeach
                    @endif
                @endslot
            @endcomponent
        ...
        <div class="col-lg-4 col-md-6">
            ...
            <div class="col-md-9 col-sm-8">
                <div class="image-input avatar xxl rounded-4" style="background-image: url({{ isset($admin) && $admin->foto_profil != NULL ? asset('upload/foto_profil/admin/'.$admin->foto_profil) : asset('assets/dashboard/images/avatar.png') }});">
                    <div class="avatar-wrapper rounded-4" style="background-image: url({{ isset($admin) && $admin->foto_profil != NULL ? asset('upload/foto_profil/admin/'.$admin->foto_profil) : asset('assets/dashboard/images/avatar.png') }});"></div>
