...
@section('content')
    <section>
        @if (isset($deteksiDini) && $deteksiDini->is_valid == 2)
            <div role="alert" class="alert alert-danger mt-2">
                <h6>Alasan data anda ditolak:</h6>
                <p class="mb-0">{{ $deteksiDini->alasan_ditolak }}</p>
            </div>
        @endif
                ...
                <div class="card p-0">
                    ...
                    <div class="card-body">
                        @component('dashboard.components.forms.utama.deteksi_dini')
                            ...
                            @slot('dataEdit', $deteksiDini)
                            @slot('form_id', 'form_deteksi_dini')
                            @slot('proses', url('proses-deteksi-dini'))
                            @slot('action', url('deteksi-dini/' . $deteksiDini->id))
                            @slot('method', 'PUT')
